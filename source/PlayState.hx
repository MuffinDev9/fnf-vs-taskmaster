...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['1 Point', 0.2], //From 0% to 19%
		['2 Points', 0.4], //From 20% to 39%
		['3 Points', 0.6], //From 50% to 59%
		['4 Points', 0.8], //From 70% to 79%
		['5 Points', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['alexhorne']);
				...
				switch(achievementName)
				{
					case 'alexhorne':
						if(isStoryMode && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'task':
									if(achievementName == 'alexhorne') unlock = true;
							}
						}
					case 'hellmode':
						if(isStoryMode && CoolUtil.difficultyString() == 'HELL' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								case 'task':
									if(achievementName == 'hellmode') unlock = true;
							}
						}
				}
